...
type CreUser = { id?: number; name?: string; username?: string; branch?: string; is_active?: boolean };

export default function ManageCrePage() {
  const [session, setSession] = useState<SessionInfo | null>(null);
  const [data, setData] = useState<CreUser[]>([]);
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        setSession(sj);
        const res = await fetch("/api/cre_users", { credentials: "include" });
        const json = await res.json();
        if (!json.success) throw new Error(json.message || "Error");
        setData(json.cre_users || []);
      } catch (e) {
