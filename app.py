...
@require_auth()
def cre_guide_pdf():
    pdf_path = os.path.join(app.config['UPLOAD_FOLDER'], 'cre_guide.pdf')
...
@require_auth()
def ps_guide_pdf():
    pdf_path = os.path.join(app.config['UPLOAD_FOLDER'], 'ps_guide.pdf')
    if os.path.exists(pdf_path):
        return send_file(pdf_path, mimetype='application/pdf')
    else:
        return 'No PS Guide PDF uploaded.', 404
